...
class OwnerControllerTests {
	...
	void testProcessFindFormByPartialName() throws Exception {
		...
		String searchString = "Geo'; DROP TABLE owners; --";
		
		Page<Owner> tasks = new PageImpl<>(Lists.newArrayList(george(), new Owner()));
		Mockito.when(this.owners.findByLastName(eq(searchString), any(Pageable.class))).thenReturn(tasks);
		mockMvc.perform(get("/owners?page=1").param("lastName", searchString))
			.andExpect(status().isOk())
